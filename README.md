...
**Chrome-to-Terminal — Web → Terminal**
...
On Linux, Chrome-to-Terminal keeps a persistent terminal session so repeated commands can be run in the same terminal with previous commands, output and working directory visible. The persistent Linux session uses `tmux` when available. On systems without `tmux`, Chrome-to-Terminal falls back to keeping each terminal window open at a prompt.
...
Chrome Extension → Chrome Native Messaging → local Chrome-to-Terminal connector → operating-system terminal.
...
For the shared-session behavior, install `tmux` once with your distribution's normal package manager. Chrome-to-Terminal creates a dedicated `chrome-to-terminal` tmux session automatically and opens a terminal attached to it. Closing the terminal does not destroy the session, so the next **Run in Terminal** can reconnect to the same history and working directory.
